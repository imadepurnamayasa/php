...
if ($csrf_token['enable']) {
	...
	// var_dump(strpos(current_url(), $config['base_url'] . 'public/'));
	// echo current_url(); die;
	if (strpos(current_url(), $config['base_url'] . 'public/') === false ) {
		csrf_settoken();
	}
}
...
if (!empty($_SESSION['user'])) {
	...
	$logged_user = $db->query($sql, $_SESSION['user']['id_user'])->getRowArray();
}
