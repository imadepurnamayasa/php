...
jQuery(document).ready(function () {
	...
	$('table').delegate('.qty-plus', 'click', function() 
	{
		...
		$(this).prev().val(format_ribuan(new_value + 1)).trigger('keyup');
	});
	...
	$('table').delegate('.qty-min', 'click', function() 
	{
		...
		if (new_value < 1)
			new_value = 1;
		$(this).next().val(format_ribuan(new_value - 1)).trigger('keyup');
	});
	
	// Kalkulasi total ter item
	$('table').delegate('.harga-satuan', 'keyup', function() 
	{
		...
		pengali = 1;
		if ( $tr.find('.pengali').length ) {
			pengali = setInt( $tr.find('.pengali').val() );
		}
		harga_total = qty * pengali * harga_satuan;
	...
	//-- Kalkulasi total peritem
	
	$('table').delegate('.pengali', 'keyup', function() 
	{
		this.value = format_ribuan(this.value, true);
		$(this).parents('tr').eq(0).find('.harga-satuan').trigger('keyup');
	})
	...
	function addBarang(barang) 
	{
		...
		if (list_barang_terpilih[barang.id_barang] != undefined) {
			...
			let $pengali = $id_barang.parents('tr').eq(0).find('.pengali');
			let pengali = parseInt($pengali.val());
			$pengali.val(pengali + 1);
	...
	$('table').delegate('.print-invoice', 'click', function(e) {
		e.preventDefault();
		const url = $(this).attr('data-url');
		window.open(url, top = 500, left = 500, width = 600, height = 600, menubar = 'no', status = 'no', titlebar = 'no'); 
		return false;
