...
class PenjualanModel extends \App\Models\BaseModel
{
	...
	{	
		...
		foreach ($_POST['id_barang'] as $key => $id_barang) 
		{
			...
			$harga_satuan = clean_number($_POST['harga_satuan'][$key]);
			$qty = clean_number($_POST['qty'][$key]);
			$qty_pengali = !empty($_POST['qty_pengali'][$key]) ? clean_number($_POST['qty_pengali'][$key]) : 1;
			$harga_barang =  $harga_satuan * $qty * $qty_pengali;
						
			$diskon_nilai = clean_number($_POST['diskon_barang_nilai'][$key]);
			...
			$untung_rugi = $harga_barang - $_POST['harga_pokok'][$key] * $qty * $qty_pengali;
			...
			$data_db_barang[$key]['harga_total'] = $harga_satuan * $qty * $qty_pengali;
		...
		$diskon_total_nilai = clean_number($_POST['diskon_total_nilai']);
		$diskon = 0;
		...
		$data_db['penyesuaian'] = clean_number($_POST['penyesuaian_nilai']);
		if (!$data_db['penyesuaian']) {
			$data_db['penyesuaian'] = 0;
		}
		...
		{
			$total_bayar += (int) clean_number($val);
		}
		...
		if ($total_bayar) 
		{
			foreach ($_POST['jml_bayar'] as $key => $val) 
			{
				...
				$data_db_bayar[$key]['jml_bayar'] = clean_number($val);
