	...
	*/
		...
		if ($this->isLoggedIn && $this->currentModule['nama_module'] != 'login') {
			...
			if ($this->moduleRole) {
				...
				if ($this->currentModule['login'] == 'Y') {
					$this->setCurrentModule('error');
					$this->data['msg']['status'] = 'error';
					$this->data['msg']['message'] = 'Anda tidak berhak mengakses halaman ini (' . $this->currentModule['judul_module'] . '). Role untuk module ini belum didefinisikan'; 
					$this->view('error.php',$this->data);
					exit();
				}
			}
	...
	protected function addJs($file, $print = false) {
		if ($print) {
			...
			if (is_array($file) ) {
				$value = ['script' => $file['file']];
				if (key_exists('attr', $file)) {
					$value['attr'] = $file['attr'];
				}
				$this->data['scripts'][] = $value;
	...
	private function checkRoleAction() 
	{
		...
		if ($this->config->checkRoleAction['enable_global']) 
		{
			...
			$list_error = ['add' => 'menambah', 'edit' => 'menubah'];
	...
	protected function hasPermission($action, $module = null, $exit = false) 
	{
