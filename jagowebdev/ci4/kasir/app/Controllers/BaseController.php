...
class BaseController extends Controller
{
	...
	*/
	public function __construct() 
	{
		...
		$this->setNotifikasiStok();
		$this->data['config'] = $this->config;
		$this->data['request'] = $this->request;
		$this->data['isloggedin'] = $this->isLoggedIn;
		$this->data['session'] = $this->session;
		$this->data['site_title'] = 'Aplikasi Kasir Jagowebdev';
		$this->data['site_desc'] = 'Aplikasi Kasir Jagowebdev lengkap dengan berbagai fitur untuk memudahkan pengembangan aplikasi';
	...
	private function setNotifikasiStok() 
	{
		$result = $this->model->getSetting('stok');
		foreach ($result as $val) {
			$setting_stok[$val['param']] = $val['value'];
		}
		
		$this->data['setting_stok'] = $setting_stok;
		if ($this->data['setting_stok']['dashboard_show'] == 'Y' || $this->data['setting_stok']['notifikasi_show'] == 'Y') {
			$this->data['global_jml_barang'] = $this->model->getTotalBarang();
			$this->data['global_stok_barang'] = $this->model->getTotalStokBarang();
		}
	}
	...
	protected function hasPermission($action, $nama_module = '', $exit = false) 
	{
		$exists = false;
		if ($nama_module) {
			foreach ($this->user['all_permission'] as $id_permission => $list_permission) {
				foreach ($list_permission as $nama_permission => $val) {
					if ($val['nama_module'] == $nama_module && $action == $nama_permission) {
						$exists = true;
						break;
					}
				}
