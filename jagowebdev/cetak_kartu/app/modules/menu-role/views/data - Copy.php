			...
			foreach ($result as $key => $val) {
				...
				echo '<tr>
						...
						<td>
							...
							<a href="' . module_url() . 'edit/?id=' . $val['module_id'] .'" class="btn btn-success btn-xs me-1"><i class="fa fa-edit pe-1"></i> Edit</a>
							<form method="post" action="'.current_url().'"><button data-action="delete-data" type="submit" class="btn btn-danger btn-xs" name="delete"><i class="fas fa-times me-1"></i> Delete</button><input type="hidden" name="id" value="'.$val['module_id'].'"/><input type="hidden" name="delete" value="delete"/></form>
							</div>
						</td>
					</tr>';
				$no++;
