			...
			if ($form_errors) {
				...
				if ($new_name_depan && $new_name_belakang) {
					...
					if ($_POST['berlaku_jenis'] == 'periode') {
						$data_db['berlaku_periode_tahun'] = $_POST['berlaku_periode_tahun'];
					} else {
...
function validate_form() 
{
	...
	$validation->setRules('berlaku_jenis', 'Berlaku', 'trim|required');
	
	if ($_POST['berlaku_jenis'] == 'custom_text') {
		$validation->setRules('berlaku_custom_text', 'Custom Text', 'required|min_length[10]');
	}
