...
jQuery(document).ready(function () 
{
	...
	$('#nama-tabel').change(function(){
		...
		$loader = $('<i>').addClass('fas fa-circle-notch fa-spin ml-2').insertAfter($('#jml-data').find('strong'));
		$.get(module_url + '/max-data', function(max_data){
			$.ajax({
			  ...
			  success: function(data){
				  ...
				  $('#jml-data').html('<strong>' + format_ribuan(data) + '</strong>');
