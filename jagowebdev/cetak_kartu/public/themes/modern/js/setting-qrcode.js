jQuery(document).ready(function () 
{
	...
	$('#posisi-kartu').change(function(e){
		// alert();
		e.preventDefault();
		// position = {};
		$form = $('#form-qrcode');
	...
	$('#content-field-database').change(function(e) {
	...
	$('#btn-preview-qrcode').click(function(e) {
		...
		let text = 'url: <a href="https://jagowebdev.com">Jagowebdev.com</a>';
		let query_string = $('form').serialize() + text;
		$.get(base_url + '/setting-qrcode/preview-qrcode?' + query_string, function(data) {
			$alert = $(data).hasClass('alert');
			if ($alert) {
				bootbox.alert(data);
			} else {
				$('.qrcode-container').empty().html(data);
			}
		})
