...
header("Content-Disposition: attachment; filename=laporan-bulanan.xls");//ganti nama sesuai keperluan
...
<table>
	...
	<tr><td style='border:none' width='210px'>Laporan sampai dengan</td> 		<td style='border:none'>: $dtu[laporan_sampai]</td></tr>
	<tr><td style='border:none'>Nama Program</td> 		  						<td style='border:none'>: $dtu[nama_program]</td></tr>
	<tr><td style='border:none'>Nama Kegiatan</td> 		   						<td style='border:none'>: $dtu[nama_kegiatan]</td></tr>
	<tr><td style='border:none'>Bidang / Sub Bag/ Seksi</td> 					<td style='border:none'>: $dtu[bidang_bagian_seksi]</td></tr>
	<tr><td style='border:none'>KPA</td> 										<td style='border:none'>: $aa[nama_lengkap]</td></tr>
	<tr><td style='border:none'>PPTK</td> 										<td style='border:none'>: $b[nama_lengkap]</td></tr>
	<tr><td style='border:none'>Pagu Anggaran</td> 								<td style='border:none'>: Rp ".format_rupiah($dtu[pagu_anggaran])."</td></tr>
</table><br>
...
<table width='100%'>
	  ...
	  while ($r = mysql_fetch_array($jenis)){
				$tt = mysql_fetch_array(mysql_query("SELECT sum(a.vol) as vol, sum(c.vol_fisik) as voltar, sum(c.persen_fisik) as persentar, sum(c.persen_keuangan) as persenkeuu, sum(d.persen_fisik) as persentarb, sum(d.persen_keuangan) as persenkeuub, sum(d.vol_fisik) as volrea, sum(a.total_rp) as total, sum(a.sisa_pagu) as sisaaa, sum(a.bobot) as bobott, sum(c.rp_keuangan) as uangtar, sum(d.rp_keuangan) as uangrea,
												(sum(c.rp_keuangan)/sum(a.total_rp))*100 as persentarkeu, (sum(d.rp_keuangan)/sum(a.total_rp))*100 as persenreakeu
	  ...
	  echo "<tr bgcolor=#000>
				...
				<td style='color:#fff;'>".number_format($ttbtar,2)."</td>
				...
				<td style='color:#fff;'>".number_format($ttbrea,2)."</td>
				<td style='color:#fff;'>".format_rupiah($tt[uangrea])."</td>
				<td style='color:#fff;'>".number_format($tt[persenkeuub],2)."</td>
		...
		while ($sr = mysql_fetch_array($subjenis)){
			$cek = mysql_num_rows(mysql_query("SELECT * FROM rb_anggaran_belanja where id_sub_jenis_belanja='$sr[id_sub_jenis_belanja]' AND id_data_umum='$_GET[id]'"));
			if ($cek >= 1){
													...
													JOIN rb_detail_target b ON a.id_anggaran_belanja=b.id_anggaran_belanja
														JOIN rb_detail_realisasi c ON a.id_anggaran_belanja=c.id_anggaran_belanja
															JOIN rb_data_umum d ON a.id_data_umum=d.id_data_umum
																where id_sub_jenis_belanja='$sr[id_sub_jenis_belanja]' AND d.id_data_umum='$_GET[id]'");
	  ...
	  $to = mysql_fetch_array(mysql_query("SELECT sum(d.persen_fisik) as fisiktot, sum(c.persen_fisik) as fisiktottar, sum(total_rp) as total, sum(bobot) as bobott, sum(c.rp_keuangan) as uangtar, sum(d.rp_keuangan) as uangrea, sum(a.sisa_pagu) as sisapaguu  
												FROM `rb_anggaran_belanja` a JOIN rb_sub_jenis_belanja b ON a.id_sub_jenis_belanja=b.id_sub_jenis_belanja 
													JOIN rb_detail_target c ON a.id_anggaran_belanja=c.id_anggaran_belanja 
														JOIN rb_detail_realisasi d ON a.id_anggaran_belanja=d.id_anggaran_belanja 
															where a.id_data_umum='$_GET[id]'"));
	 ...
	 echo "<tr bgcolor=#ff6666>
				...
				<td></td>
				<td></td>
				<td>".number_format($fisitotart,2)."</td>
				<td></td>
				<td>".format_rupiah($to[uangtar])."</td>
				<td>".number_format($tottarpersen,2)."</td>
				<td></td>
				<td></td>
				<td>".number_format($fisitot,2)."</td>
				<td></td>
				<td>".format_rupiah($to[uangrea])."</td>
				<td>".number_format($totreapersen,2)."</td>
				<td></td>
...
<table width=100%>
  ...
  <tr>
    <td colspan='3' style='border:none' width="230" align="center">MENGETAHUI / MENYETUJUI : <br> Kuasa Pengguna Anggaran,</td>
  ...
  <tr>
    <td colspan='3' style='border:none' align="center"><br /><br /><br />
